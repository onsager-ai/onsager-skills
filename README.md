...
`plan-dag` has no Onsager dependency; it only needs `graph-easy` on PATH for the boxart/ascii render targets (`cpan -T -i Graph::Easy`, or `apt install libgraph-easy-perl` on Debian/Ubuntu).
