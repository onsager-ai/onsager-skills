...
- Surface rules:
  ...
  - **Claude Code on the web, claude.ai web / mobile (HTML-capable, no real terminal pane):** prefer the HTML path. Render with `--as=html --out /tmp/plan-dag.html` and send the file via `SendUserFile` so the user sees a scalable diagram with the critical-path footer baked in. Status is communicated by the per-node fill colors and leading emoji (the styled SVG is self-explanatory — no separate legend). PNG (`--as=png --out /tmp/plan-dag.png`) is a fine fallback on surfaces that render inline images but not HTML. Add prose commentary below the file — critical path, next pickable node — but don't echo the text-art version too.
